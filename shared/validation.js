  ...
  switch (action.type) {
    ...
    case ActionType.BUY: {
      ...
      if (action.itemId === "feed") {
        // Buy animal feed
        const totalCost = Math.max(1, Math.floor(FEED_COST * globalThis.GAME_CONFIG.BUY_MULTIPLIER)) * action.quantity;
      ...
      const totalCost = Math.max(1, Math.floor(seed.packCost * globalThis.GAME_CONFIG.BUY_MULTIPLIER)) * action.quantity;
      if (state.coins < totalCost)
        return { valid: false, reason: "Not enough coins" };
    ...
    case ActionType.BUY_SLOT: {
      ...
      const cost = Math.max(1, Math.floor(getSlotUnlockCost(state.unlockedSlots) * globalThis.GAME_CONFIG.BUY_MULTIPLIER));
      if (state.coins < cost)
    ...
    case ActionType.BUY_ANIMAL: {
      ...
      if (state.coins < Math.max(1, Math.floor(animalDef.cost * globalThis.GAME_CONFIG.BUY_MULTIPLIER)))
        return { valid: false, reason: "Not enough coins" };

      state.coins -= Math.max(1, Math.floor(animalDef.cost * globalThis.GAME_CONFIG.BUY_MULTIPLIER));
    ...
    case ActionType.SELL: {
      ...
      state.coins += Math.max(1, Math.floor(itemDef.sellPrice * globalThis.GAME_CONFIG.SELL_MULTIPLIER)) * action.quantity;
      return { valid: true };
    }
    ...
    case ActionType.SELL_ALL: {
      ...
      for (const item of state.warehouse) {
        ...
        if (def && def.sellPrice > 0) {
          totalGold += Math.max(1, Math.floor(def.sellPrice * globalThis.GAME_CONFIG.SELL_MULTIPLIER)) * item.quantity;
        }
      }
